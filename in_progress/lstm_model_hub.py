...
import numpy as np
from encoder import TFHubEncoder


class LSTMModel(object):

  def __init__(self, config, pretrained_embedding=None):
    self._input         = tf.placeholder(dtype=tf.string,shape=[None],name='input')
    ...
    outputs = TFHubEncoder(self._input).get_embedded_output()
    self.size = outputs.get_shape().as_list()[-1]
...
class Trainer(object):
    def __init__(self,config,X,y,embedding=None):
...
if __name__ == '__main__':
    ...
    data_processor = DataProcessor(data_path,vocab_size=config['vocab_size'],\
                    seperator=',',max_seq_len=config['num_steps'],header=0,reverse=True,raw_data=True)
    ...
    trainer = Trainer(config,data,labels)
