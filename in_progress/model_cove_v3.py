...
import numpy as np
from encoder import LSTMEncoder,  CoVeEncoder


class CoVeModel(object):
  def __init__(self, config, pretrained_embedding):
    self._input         = tf.placeholder(dtype=tf.int32,shape=[None,config['num_steps'],config['embed_size']],name='input')
    ...
    outputs = LSTMEncoder(self._input,self._input.get_shape().as_list()[-1],\
                             self.size,\
    ...
    tvars    = [i for i in tf.trainable_variables() if 'CoVe' not in i.name]
...
class Trainer(object):
    def __init__(self,config,X,y,embedding):
        ...
        self.cove_model = tf.keras.models.load_model('saved_models/CoVe/Keras_CoVe.h5')
        self.model = CoVeModel(config,embedding)
        self.train_op = self.model.train_op
        config_temp = tf.ConfigProto(
        device_count = {'GPU': 0}
        )
        self.sess = tf.Session(config=config_temp)
        self.sess.run(tf.global_variables_initializer())
        self.model.cove_model.load_weights(self.sess)
    ...
    def run_epoch(self,X,y, training=False):
        ...
        for i in range(int(len(X)/self.batch_size)+1):
            ...
            X_temp = self.sess.run(self.temp_output,{self.temp_input:X_sub})
...
def get_input(train_file_name, test_file_name):
