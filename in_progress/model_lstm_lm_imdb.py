...
import numpy as np
from encoder import LSTMEncoderWithEmbedding, CNNEncoderWithEmbedding, LSTMEncoder
...
class LSTMModel(object):
  ...
  def __init__(self, config, pretrained_embedding):
    ...
    with tf.device("/cpu:0"):
        embedding = tf.get_variable("embedding", [config['vocab_size']+1, self.embed_size], dtype=tf.float32)
    ...
    self.lm_loss = self.get_lm_graph(self._input,self.encoder_outputs,embedding)
  ...
  def get_classification_graph(self):
    ...
    l2_loss =  sum(tf.nn.l2_loss(tf_var)
      for tf_var in tf.trainable_variables()
      )
    clf_cost = tf.reduce_mean(loss) + self.weight_decay*l2_loss
    predicted_class = tf.cast(tf.argmax(tf.nn.softmax(logits),axis=-1),tf.int32)
    return clf_cost, predicted_class


  def get_lm_graph(self,X,encoder_output,embedding):
    #   encoder_output = tf.stack(encoder_output,axis=1)
      encoder_output = tf.reshape(encoder_output,[-1,encoder_output.get_shape().as_list()[-1]])
      y_lm = tf.concat([X[:,1:] , tf.zeros([tf.shape(X)[0],1 ],dtype=tf.int32)],axis=1)
      print(y_lm.get_shape())
      y_lm = tf.reshape(y_lm,[-1])
      mask = tf.cast(y_lm > 0,tf.float32)
      mask = tf.reshape(mask,[-1])
      logits    = tf.matmul(encoder_output, tf.transpose(embedding,[1,0]))
      loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y_lm,logits=logits)
      loss = tf.multiply(loss,mask)
      lm_cost = tf.reduce_sum(loss)/tf.reduce_sum(mask)
      return lm_cost
...
class Trainer(object):
    def __init__(self,config,X,y,embedding):
        ...
        self.cl_train_op = self.model.cl_train_op
        self.lm_train_op = self.model.lm_train_op
    ...
    def train(self):
        ...
        for idx in range(self.max_max_epoch):
            ...
            train_cost = self.run_epoch_lm(self.X_train,self.y_train, training=True)
            # print('Cost %0:.2f , Accuracy: 0:.2f'.format(cost,accuracy))
            val_cost = self.run_epoch_lm(self.X_test,self.y_test)
            cost_history.append(val_cost)
            print(val_cost)
            print('BATCH %d |Validation: Cost %f , Perplexity: %f ' \
                            %(idx,\
                                round(val_cost,2),np.exp(float(val_cost))))
    ...
    def run_epoch(self,X,y, training=False):
        ...
        ops        = [self.model.cl_loss, self.model.predict]
        if training:
            feed_dict[self.model.keep_prob] = self.keep_prob
            ops = [self.cl_train_op,self.model.cl_loss, self.model.predict]
    ...
    def run_epoch_lm(self,X,y, training=False):
        temp_cost  = []
        temp_acc   = []
        feed_dict  = {self.model.keep_prob: 1}
        ops        = [self.model.lm_loss]
        if training:
            feed_dict[self.model.keep_prob] = self.keep_prob
            ops = [self.lm_train_op,self.model.lm_loss]
        for i in range(int(len(X)/self.batch_size)+1):
            X_sub = X[i*self.batch_size: (i+1)*self.batch_size,:]
            y_sub = y[i*self.batch_size: (i+1)*self.batch_size]
            feed_dict[self.model.input] = X_sub
