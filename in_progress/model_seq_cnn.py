...
class CNNModel(object):
  ...
  def __init__(self, config, pretrained_embedding):
    ...
    self.cnn_model = seqCNN(num_classes=config['num_classes'], num_filters=100,\
                        num_pooled, vocabulary_length=config['vocab_size'],\
                        region_size=3, max_sentence_length=self.num_steps)

    logits = self.cnn_model.logits
