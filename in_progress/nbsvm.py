'''
Source:
https://github.com/Joshua-Chin/nbsvm/blob/master/nbsvm/__init__.py
'''
import numpy as np
...
class NBSVM(BaseEstimator, LinearClassifierMixin, SparseCoefMixin):
    ...
    def _fit_binary(self, X, y):
        p = np.asarray(self.alpha + X[y == 1].sum(axis=0),dtype=np.float32).flatten()
        q = np.asarray(self.alpha + X[y == 0].sum(axis=0),dtype=np.float32).flatten()
